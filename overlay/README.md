...
| `usr/local/bin/a50-device-setup.sh` (clock block) | The lock screen and panel clock showed the wrong time while Settings showed the right one. `lomiri-indicators.target` starts the indicators from `/etc/systemd/user/lomiri-indicators.target.wants/`, and `ayatana-indicator-datetime.service` is the one unit missing from it — its only `WantedBy=` is `ayatana-indicators.target`, which never activates. 7 of 8 indicators ran; the clock never started and logged nothing. The system clock itself was always correct. |
...
| `usr/lib/udev/rules.d/99-a50-fimc-scan-block.rules` | GStreamer's plugin scanner (spawned each session by `clean-gstreamer-cache.service` + media-hub) probes every `/dev/video*`, and the fimc-is2 group-close path can wedge it in an unkillable D-state — which blocks the PM freezer outright and precedes kernel memory corruption. `TAG-="uaccess"` is the load-bearing clause: without it logind hands phablet an ACL that overrides every mode/owner. Nothing host-side needs these nodes — the camera goes through the container's HAL, which applies its own `cameraserver:camera` permissions inside. [018](../docs/experiments/018-suspend-freezer-wifi-instability.md) |
