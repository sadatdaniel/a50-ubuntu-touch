    ...
    Component {
        ...
        ShellDialog {
            ...
            Button {
                ...
                onClicked: {
                    // a50: hide() here destroyed QML content while this
                    // handler was still on the stack ("Object destroyed
                    // while one of its QML signal handlers is in progress",
                    // Dialogs.qml:277) and the session aborted ~20 s later
                    // in QQmlData::destroyed's qFatal. Defer the whole
                    // sequence; capture() grabs on a later frame, after the
                    // dialog is gone, so the screenshot is unchanged.
