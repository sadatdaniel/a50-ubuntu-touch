...
| **The signal indicator is blank while idle** | Cosmetic, and calls and data both work. Measured: RSRP is -108..-112 dBm idle and the plugin reports **1%**, but 27% during a call. 1% is not what the RSRP map alone gives — `{-115 → 1%, -105 → 33%}` with interpolation puts -110 dBm at ~17%. The likely cause is `binder_netreg_tech_percent()` taking the **minimum** across every available measurement, so one poor indicator (RSRQ or RSSNR) drags it to 1% regardless of RSRP. **Not confirmed** — proving it needs an ofono debug capture, which drops the data connection, and the packaged plugin may differ from upstream master. |
