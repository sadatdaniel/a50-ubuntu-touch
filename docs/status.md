...
| **GPS gets no fix** | Permissions are solved and apps register as clients, but `gpsd` never initialises: one thread in `nanosleep`, no vendor libs loaded, never opens `/dev/gnss_ipc`. [experiment 009](experiments/009-gps-permissions.md) |
