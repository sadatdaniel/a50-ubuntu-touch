...
| Open | Why it matters |
|---|---|
| **Suspend panics the kernel — and so does opening the camera** | The device never sleeps; entering suspend killed it. `pm_test` narrowed it: `freezer` passes, `devices` crashed. The crash record names `fimc_is_devicemgr_open+0x1e8`, and disassembly puts the fault on `sensor->vctx->next_device = ischain` with `vctx` dangling, not NULL — `fimc_is_sensor_close()` never clears it. **A patch exists (a50-halium `fimc-is-clear-vctx-on-close`) and does NOT fix it**: the panic recurred twice with that kernel installed. The full stack shows the real trigger is `SyS_openat` on a V4L2 node ~23 s into boot — the container's Android camera HAL — not suspend. Suspend is separately blocked at `PM_SUSPEND_PREPARE` by an ABOX veto (`abox_pm_notifier`, `-EBUSY`), clearable by stopping ABOX dump 0, after which the device suspends and **does not resume**. Full suspend has never completed. Idle drain meanwhile is ~38 mA. [017](experiments/017-suspend-camera-panic.md) |
