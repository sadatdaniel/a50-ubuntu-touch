...
| **Waydroid runs Android apps** | Android 13 (LineageOS 20 VANILLA) with the matching `HALIUM_11` vendor image; **F-Droid installed and running**, confirmed on screen. The only blocker was binder: Waydroid needs its own domain, the Halium container owns `binder/hwbinder/vndbinder`, and this 4.14 kernel has no binderfs - so `anbox-binder,anbox-hwbinder,anbox-vndbinder` were compiled in. No regressions: lightdm, the Halium container, GPS, Bluetooth, telephony and audio all still fine alongside it. [experiment 013](experiments/013-waydroid.md) |
