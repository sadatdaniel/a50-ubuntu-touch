...
| **Fingerprint capture** | The optical ET713 can only image with the screen lighting the finger (HBM). Read from the kernel source: the panel applies `mask_brightness` only when `decon->current_mask_layer` is true, and `decon_get_mask_layer()` sets that **only** when the compositor submits a window with `state == DECON_WIN_STATE_FINGERPRINT`. Lomiri/Mir never does, so `actual_mask_brightness` stays 0 and `nd cnt` never rises. `CONFIG_SUPPORT_MASK_LAYER` *is* built in - the machinery exists, it is just never triggered. Fixable via a kernel patch exposing the mask layer to userspace. [experiment 012](experiments/012-fingerprint.md) |
