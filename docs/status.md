...
| Open | Why it matters |
|---|---|
| **Suspend panics the kernel** | The device never sleeps, and entering suspend kills it. Narrowed with `pm_test`: the `freezer` phase passes, the `devices` phase crashes every time. The bootloader's crash record names it — `"PC":"fimc_is_devicemgr_open+0x1e8/0x3a0"`, `"RR":"KP"` — and disassembly puts the fault on `sensor->vctx->next_device = ischain`, where `vctx` is dangling rather than NULL because `fimc_is_sensor_close()` never clears it. Userspace is not at fault: `ISystemSuspend` is registered, repowerd is bound to it, and no wakelock blocks. A patch is written (a50-halium `fimc-is-clear-vctx-on-close`) and **not yet boot-tested**. Idle drain without suspend measures ~38 mA. [017](experiments/017-suspend-camera-panic.md) |
