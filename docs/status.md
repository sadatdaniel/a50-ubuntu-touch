...
| **Fingerprint capture** | The EGISTEC ET713 is an optical under-display sensor; it can only image with the screen lighting the finger spot (HBM). `actual_mask_brightness` stays 0 because Samsung's decon mask layer is composited by SurfaceFlinger, not Lomiri/Mir. So `nd cnt` never rises and no fingerprint is read. Compositor-level work. [experiment 012](experiments/012-fingerprint.md) |
