...
profile a50-aa10-enforcement-probe {
  #include <abstractions/base>
  /usr/bin/cat rix,
  /userdata/a50-session20-aa10/probe-allowed r,
  deny /userdata/a50-session20-aa10/probe-denied r,
}
PROFILE
apparmor_parser -a "$P"
trap 'apparmor_parser -R "$P"' EXIT
test "$(aa-exec -p a50-aa10-enforcement-probe -- /usr/bin/cat "$B/probe-allowed")" = allowed
if aa-exec -p a50-aa10-enforcement-probe -- /usr/bin/cat "$B/probe-denied" > "$B/denied-result" 2> "$B/denied-error"; then
