...
level=${1:?Specify platform, processors or core}
cycle=${2:?Specify cycle 1, 2 or 3}
case "$level" in platform|processors|core) ;; *) exit 2 ;; esac
...
sync
printf '<5>A50 session20 aa12: USB-attached %s cycle %s begins\n' "$level" "$cycle" > /dev/kmsg
